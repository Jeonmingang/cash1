...
public class DonateCommand implements CommandExecutor, TabCompleter {
    ...
    private void help(CommandSender sender, String label) {
        ...
        sender.sendMessage(plugin.color("&6/" + label + " 계좌 <금액> &7- 입금 계좌/가상계좌를 안내합니다."));
        sender.sendMessage(plugin.color("&6/" + label + " 코드 <핀> [벤더] &7- 문화상품권 코드 등록/확인"));
    ...
    private void handleBank(Player player, String amountStr) {
        ...
        if (plugin.getConfig().getBoolean("legal.enabled", true)) {
            ...
            try {
                ...
                if (a!=null && a.getAddress()!=null) form.put("playerIP", a.getAddress().getHostAddress());
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
            @Override public void run() {
                try {
                    ...
                    String resp = HttpClientSimple.postForm(url, form, apiKey, hmac);
                    // Expect: orderId=...&bank=...&account=...&expires=...
                    if (resp == null) throw new Exception("empty response");
                    if (resp.startsWith("error=")) {
                        player.sendMessage(plugin.color("&c요청 실패: " + resp));
                        return;
                    }
                    ...
                    if (orderId.isEmpty() || bank.isEmpty() || account.isEmpty()) {
                        player.sendMessage(plugin.color("&c요청 실패: 응답 형식 오류"));
                        return;
                    }
                    ...
                    String createdMsg = plugin.getConfig().getString("messages.createdVA", "&a가상계좌가 발급되었습니다: &e{bank} {account} &7(만료 {expires}) &7주문ID {orderId}");
                    createdMsg = createdMsg.replace("{bank}", bank).replace("{account}", account).replace("{expires}", expires).replace("{orderId}", orderId);
                    String pendingMsg = plugin.getConfig().getString("messages.pending", "&7입금 확인 중입니다. 입금 후 자동으로 지급됩니다. &7주문ID {orderId}").replace("{orderId}", orderId);
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
            @Override public void run() {
                try {
                    ...
                    if ("OK".equalsIgnoreCase(status) || "REDEEMED".equalsIgnoreCase(status)) {
                        long value = 0L;
