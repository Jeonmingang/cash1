...
public class DonateAutomationPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (!okVault) {
            log("&eVault/경제 플러그인을 찾지 못했습니다. 캐시 명령으로만 정산합니다.");
        }

        // Commands (register by Korean primary names; fall back to English aliases)
        DonateCommand donate = new DonateCommand(this);
        DonateAdminCommand donateAdmin = new DonateAdminCommand(this);

        PluginCommand donateCmd = this.getCommand("후원");
        if (donateCmd == null) donateCmd = this.getCommand("donate");
        if (donateCmd != null) {
            donateCmd.setExecutor(donate);
            donateCmd.setTabCompleter(donate);
        } else {
            log("&cplugin.yml에 '후원' 명령어가 등록되지 않았습니다.");
        }

        PluginCommand donateAdminCmd = this.getCommand("후원관리");
        if (donateAdminCmd == null) donateAdminCmd = this.getCommand("donateadmin");
        if (donateAdminCmd != null) {
            donateAdminCmd.setExecutor(donateAdmin);
            donateAdminCmd.setTabCompleter(donateAdmin);
        } else {
            log("&cplugin.yml에 '후원관리' 명령어가 등록되지 않았습니다.");
        }
        ...
        log("&aUltimateDonateAutomation 활성화 완료.");
    }
    ...
    public void giveRewardNow(String playerName, long amountWon, String reason) {
        ...
        String rewardMsg = getConfig().getString("messages.rewardGiven", "&a{cash} 캐시가 지급되었습니다. (배수 {mult}, 원화 {won})");
        rewardMsg = rewardMsg.replace("{cash}", String.valueOf(cash))
                .replace("{mult}", String.valueOf(mult))
                .replace("{won}", String.valueOf(amountWon));
        Player target = Bukkit.getPlayerExact(playerName);
if (target != null && target.isOnline()) {
            target.sendMessage(ChatColor.translateAlternateColorCodes('&', prefix + rewardMsg));
        }
